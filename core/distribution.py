...
def artifact_metadata(downloads_dir: str, public_base_url: str, filename: str) -> dict:
    ...
    if not path.is_file():
        return {"filename": filename, "available": False, "url": None, "sha256": None, "size_bytes": 0}
    return {
        "filename": filename,
        "available": True,
...
def distribution_manifest(downloads_dir: str, public_base_url: str) -> dict:
    ...
    build_manifest = artifact_metadata(downloads_dir, public_base_url, WINDOWS_BUILD_MANIFEST)
    ...
    cmd_ready = bool(exe_ready and launcher["available"] and launcher["url"] and launcher["sha256"])
    return {
